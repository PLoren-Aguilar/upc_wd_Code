...
!--Pointers
!
      REAL(8), DIMENSION(:), POINTER :: rx, ry, rz, qxx, qyy, qzz, qxy, &
                                        qyz, qzx
      INTEGER, DIMENSION(:), POINTER :: list, nay
!
!--Local variables
!
      REAL(8), DIMENSION(6*mmax), TARGET :: quad
      REAL(8), DIMENSION(3*mmax), TARGET :: r
      REAL(8), DIMENSION(mmax) :: em, qrad, hnode
      REAL(8), DIMENSION(nbody) :: xx, yy, zz, emm
#ifdef MPI
      REAL(8), DIMENSION((ndim+1)*npart) :: sentarray, receivearray
#endif MPI
      REAL(8), DIMENSION(100) :: xmap, ymap, zmap
      REAL(8) :: t1, t2, t3, rr, rr05, rr2, rrx, rry, rrz, hm, dxi, dyi,&
                 dzi, rcomp2, qradn2, hsum2, hn, eps2, frx, fry, frz,   &
                 pot, frxl, fryl, frzl, potl, difx, dify, difz,         &
                 accpar, fl, fll, emred, ddd, xnp, ynp, znp,d, dmin
      ...
      INTEGER :: i, j, k, p, q, s, m, n, nlist, nlistga, nlistgn,       &
                 ...
                 icjy, jz, icjz, lk, nwal, mlocal
      INTEGER :: nvec, nsig, nmin, nmax
#ifdef openmp 
...
!$OMP DO SCHEDULE(runtime)
      DO 10 j=1,nbody
         ...
         idau(j)  = 0
         qrad(j)  = 0d0
         hnode(j) = 4d0*xyzhm(4,j)*xyzhm(4,j)
         qxx(j)   = 0d0
         qyy(j)   = 0d0
         qzz(j)   = 0d0
         qxy(j)   = 0d0
...
!
      icbrt=int(hashfac*float(nactive)**(1d0/3d0))
      icbrt1=icbrt-1
      nhash=icbrt**3d0
...
!$OMP shared(ihash,next,xmap,ymap,zmap) private(np,mm,iz,iy,ix,nc)               &
...
!$OMP shared(qxy,qzx,qyz,rank,gxyzu,partype,factor,nbody,nbmax)            &
!$OMP private(m,s,nlist,q,rrx,rry,rrz,hm,mlocal)                           &
!$OMP private(pot,eps2,nnodes,node,knodes,i,n,hn,dxi,dyi,dzi,rr2)          &
!$OMP private(rcomp2,qradn2,j,isubnode,nearl,k,rr,rr05)                    &
!$OMP private(frx,fry,frz,frxl,fryl,frzl,potl)
!$OMP DO SCHEDULE(runtime)
            ...
            DO 180 i=1,nnodes
               ...
               ELSE
                  IF (n.NE.m) THEN
                     ...
                     pot = pot + potl
                  ENDIF
                  IF ((dsqrt(rr2).LT.(0.5*(dsqrt(hm)+dsqrt(hn)))).AND.  &
                      (nlist.LT.nbmax)) THEN
                     nlist        = nlist + 1
                     nearl(nlist) = n
                  ENDIF
...
!
         IF ((nlist >= nbmax) .OR. (nlist == 0)) THEN
            ! Instead of simply killing the particle we could try to do
            ! something clever... The best method I can think of now is
            ! to manually select the nbmax closest particles from the list.
            ! The calculated gravity force for those particles should be
            ! OK. Having an excess of neighbours only means that the
            ! contribution from an excessively high number of particles would have
         ...
         ELSE
           ...
           ENDDO
         ENDIF
         !IF ((nlist.GE.nbmax).OR.(nlist.EQ.0)) THEN
         !   !PRINT*, 'nlist > nbmax!!!'
         !   !PRINT*, rank,m,mlocal,xyzhm(1,m),xyzhm(2,m),xyzhm(3,m),     &
         !   !        xyzhm(4,m),partype(m),nlist,nbmax
         !   gxyzu(4,mlocal) = 123456789 ! Mark the particles to be killed
         !   nlist = nbmax - 1
         !ENDIF
!
         ...
         !DO 220 s=1,nlist
         !   nb(s+1,mlocal) = nearl(s)
         !   IF (nb(s+1,mlocal).EQ.0) PRINT*, 'treeconst!!!!!', m,partype(m)
!220     !ENDDO
      ENDDO partloop
!$OMP END DO
!$OMP END PARALLEL
...
!$OMP DO SCHEDULE(runtime)
      DO p=1,nbody
         IF (gxyzu(4,p).EQ.123456789) THEN
            partype(p) = 2
         ENDIF
      ENDDO
!$OMP END DO
...
!
#ifdef MPI
       CALL MPI_REDUCE(nmin, globnmin, 1, MPI_INTEGER, MPI_MIN, MASTER, &
                       MPI_COMM_WORLD, ierr)
       CALL MPI_REDUCE(nmax, globnmax, 1, MPI_INTEGER, MPI_MAX, MASTER, &
                       MPI_COMM_WORLD, ierr)
       CALL MPI_REDUCE(nvec, globnvec, 1, MPI_INTEGER, MPI_SUM, MASTER, &
                       MPI_COMM_WORLD, ierr)
#else
       globnmin  = nmin
       globnmax  = nmax
