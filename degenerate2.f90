...
!
      ...
      css, cvs, dPdT, cps, partype, rank, RELFLAG, eosflag
      USE mod_EOS
!
...
!
         ...
         jhi_eos = 1
         CALL helmeos
         IF (rank.EQ.MASTER.AND.eosfail.EQV..true.) PRINT*,'EOS in   &
             particle: ',p,' failed'
!
!--New temperature
!
...
!  etot_row
!
      IF (eosflag(p).EQ.2) THEN  ! Temperature as input
          vxyzut(4,p) = etot_row(1)*(unm/uen)
          !error(p) = 0.0d0
